...
#'                alpha = 0.3, alpha_points = 1, alpha_residuals = 0.5, size_points = 2, size_rugs = 0.5)
#'
#' plot(cp_rf_y1, show_profiles = TRUE, show_observations = TRUE, show_rugs = TRUE,
#'                show_residuals = TRUE, alpha = 0.5, size_points = 3, alpha_points = 1, size_rugs = 0.5)
#'
...
#'
#' plot(cp_rf_y, show_profiles = TRUE, show_observations = TRUE, show_rugs = TRUE,
#'                show_residuals = TRUE, alpha = 0.5, color = "surface") +
#'                scale_color_gradient(low = "darkblue", high = "darkred", size_rugs = 0.5)
#'
#' plot(cp_rf_y1, show_profiles = TRUE, show_observations = TRUE, show_rugs = TRUE,
...
#'
#' plot(cp_rf_y, show_profiles = TRUE, show_observations = TRUE, size_points = 2,
...
plot.ceteris_paribus_explainer <- function(x, ...,
  ...
  if (show_profiles & is.null(aggregate_profiles)) {
    if (is_color_a_variable) {
      pl <- pl + geom_line(aes_string(color = color), size = size, alpha = alpha)
    } else {
      pl <- pl + geom_line(size = size, alpha = alpha, color = color)
    }
  }
...
"+.plot_ceteris_paribus_explainer" <- function(e1, e2) {
  ...
  class(e1) <- class(e1)[-1]
  tmp <- e2(e1)
  class(tmp) <- c("plot_ceteris_paribus_explainer", class(tmp))
  tmp
}
